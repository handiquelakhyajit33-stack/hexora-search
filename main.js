...
try{const r=await fetch("/api/search?q="+encodeURIComponent(query));const d=await r.json();if(!r.ok)throw new Error(d.error||"Search failed");meta.textContent=`About ${d.total} indexed results`;
