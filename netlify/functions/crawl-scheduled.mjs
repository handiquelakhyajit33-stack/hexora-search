...
export default async()=>{
  ...
  await supabase.from("crawl_queue").upsert(seeds.map(url=>({url,status:"queued"})),{onConflict:"url"});

  const {data}=await supabase.from("crawl_queue").select("url").eq("status","queued").limit(4);
